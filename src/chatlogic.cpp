...
ChatLogic::ChatLogic()
{
    ...
    // _chatBot will be created elsewhere. No actions are needed here.
...
ChatLogic::~ChatLogic()
{
    ...
    // _chatBot is a pointer linked to an object that is manages elsewhere.
    // nodes are defined as smart unique pointers and will be deleted once 
    // they are out of scope.
    // edges are not owned by ChatLogic.
...
void ChatLogic::LoadAnswerGraphFromFile(std::string filename)
{
            ...
            if (type != tokens.end())
            {
                ...
                if (idToken != tokens.end())
                {
                    ...
                    if (type->second == "NODE")
                    {
                        ...
                        // The type of element changed to unique_ptr<GraphNode>
                        auto newNode = std::find_if(_nodes.begin(), _nodes.end(), [&id](std::unique_ptr<GraphNode> &node) { return node->GetID() == id; });

                        // create new element if ID does not yet exist
                        if (newNode == _nodes.end())
                        {
                            // The type of element changed to unique_ptr<GraphNode> created by make_unique
    ...
    for (auto it = std::begin(_nodes); it != std::end(_nodes); ++it)
    {
        ...
        if ((*it)->GetNumberOfParents() == 0)
        {
            ...
            if (rootNode == nullptr)
            {
                rootNode = (*it).get(); // assign current node to root, by reference got from method get()
            }
            else
            {
    ...
    // Create a chatbot and link to the data
    ChatBot chatbot("../images/chatbot.png");
    chatbot.SetChatLogicHandle(this);
    chatbot.SetRootNode(rootNode);
    // move the chatBot to the rootNode. Note that the move semantics is used, so that the resource is not copied. 
